...
describe('API 轉換 production config', () => {
  ...
  it('processCSSUrls 驗證', () => {
    const publicPath = () => '../';
    const { config } = espack.processCSSUrls(false).processCSSUrls(publicPath);
    ...
    rules.forEach((rule) => {
      ...
      const miniCSSExtractPlugin = rule.use.filter(cssRule => cssRule.loader.indexOf('mini-css-extract-plugin') > -1)[0];
      if (miniCSSExtractPlugin) expect(miniCSSExtractPlugin.options.publicPath).toBe(publicPath);
    });
  });
