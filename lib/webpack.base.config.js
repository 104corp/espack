...
const util = {
  ...
  /**
   * 修改替換 CSS 中 url 位置字串
   * @param {string} cssAssetsPath - 自定路徑
   * @param {string} staticFolder - 靜態資源根目錄
   * @returns {(getPath: string) => string}
   */
  getAssetsPublicPath: (cssAssetsPath, staticFolder) => (getPath) => {
    if (cssAssetsPath !== '') {
      const replacePath = staticFolder === '.' ? '' : `${staticFolder}/`;
      return getPath.replace(replacePath, util.checkSlash(cssAssetsPath, 'end') ? cssAssetsPath : `${cssAssetsPath}/`);
    }

    if (util.checkSlash(getPath) || util.checkHttp(getPath)) return getPath;
    return `/${getPath}`;
  },
...
exports.config = ({
  ...
  const commonCssProcess = util.cssProcess(workDirectory);
          ...
          use: [
            {
              ...
              options: {
                ...
                // publicPath 處理會在 name 處理完之後，name 如果不為相對路徑就不會進入處理
                // 這邊也被拿來處理最終寫入到文件中的 url 結果，在此處任意替換路徑都不會影響真實輸出結果
                publicPath: util.getAssetsPublicPath(cssAssetsPath, staticFolder),
              },
            },
          ],
        }, {
