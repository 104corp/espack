...
const util = {
  ...
  moduleFilename: outputFileName => (
    ({ name }) => {
      const splitNames = name.split('js/');
      const cssFolder = `${splitNames[0]}css/`;
      const folderSplitNames = splitNames.splice(1).join('js/').split('/');
      const fileName = outputFileName.css.replace('[name]', folderSplitNames.pop());
      return `${cssFolder}${folderSplitNames.join('/')}${folderSplitNames.length > 0 ? '/' : ''}${fileName}`;
    }
  ),
};
exports.util = util;
  ...
  const config = {
    ...
    plugins: [
      new MiniCSSExtractPlugin({
        moduleFilename: util.moduleFilename(outputFileName),
      }),
